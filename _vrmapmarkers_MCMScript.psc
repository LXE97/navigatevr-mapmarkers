...
event OnPageReset(String a_page)
	if (error)
		...
		elseif (a_page == "Styles")
			...
			AddMenuOptionST("OID_iPlayerStyle", "Player Marker Symbol", BorderStyles[iPlayerStyle.GetValueInt()])
...
function ReadJSONFile()
    ...
    SettingsFile = JValue.readFromFile("Data/SKSE/Plugins/" + dllname + ".json")
        debug.trace("Reading json file")
	if (SettingsFile == 0)
    debug.trace("error in json file")
		error = true
	else
		; check if profile exists, choose Default if not
		int Profile = JMap.GetObj(SettingsFile, SelectedProfile)
        debug.trace("checking json profile")
		if (Profile == 0)
        debug.trace("profile not found")
